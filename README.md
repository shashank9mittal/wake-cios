...
- **Shows the customer's side of the story.** A live storefront view flips the moment a prompt PR merges — judges watch *cause* (the merge) and *effect* (what the customer now sees, and the regression it triggers) on two screens at once.
...
2. Within **≤10 seconds**, Wake's poller detects the merge, classifies it as a *prompt* change from the file diff, and auto-starts monitoring.
...
<!-- ▸▸▸ SCREENSHOT: dashboard with fired signal + observation card ◂◂◂ -->
<!-- ▸▸▸ SCREENSHOT: storefront V1 → V2 flip with PR-merged banner ◂◂◂ -->
...
- **V1 (concise):** *"Great value. Highly rated. Ships free."*
...
- **Deterministic z-score analysis** over per-metric rolling baselines — same input, same answer, every time. **No LLM anywhere in the detection path**; Claude is only invoked *after* a signal fires, to explain it.
...
- **Direction-aware**: improvements never page anyone. A deploy that *raises* checkout conversion is reported as positive drift, not an alert.
...
- The **responsible change event** (the merged PR) — *and why the others were ruled out.* The timeline tool deliberately includes **two innocent decoy changes**; the agent must pin the culprit by correlating deploy timing, affected surface, and user segment. It is never told the answer.
- A plain-English **mechanism hypothesis** (e.g., *"submit button pushed below the fold on small iOS viewports"*).
...
**Prerequisites:** Python 3.11+, Node 18+, an Anthropic API key. *(GitHub token optional — only needed for the live GHE poller.)*
...
*Walmart loses $340K an hour when a deploy quietly breaks customer behavior.<br/>Wake finds it in 8 minutes — not 7 days.*
